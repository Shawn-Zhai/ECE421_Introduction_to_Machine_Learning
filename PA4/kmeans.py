...
Original file is located at
    https://colab.research.google.com/drive/1-Syjp_D6hwGkMQFmo7vfIQfTh7R7kYwy
"""
...
def train_kmean_torch(train_data, k = 5, lr=0.1, epoch=150):
  #Define a cluster
  m = torch.rand((k, train_data.shape[1]), requires_grad=True) #k x D
  
  #Convert training data to a torch tensor
  X_train = torch.from_numpy(train_data) #N x D
  ...
  for epoch in range(epoch):
    ...
    for i in range(k):
      ...
      SqDist = torch.sum((X_train - m[i])**2, axis = 1) #sum along rows
      list_mse.append(SqDist)  

    #TODO: calculate the loss function. Complete the 
    #below 3 lines. 
    list_mse_torch = torch.stack(list_mse) #Stacks the list_mse[i]
    list_mse_torch_min,_ = torch.min(list_mse_torch, 0) #Calculate the min 
...
def evaluate(test_data, m):
  ...
  # follow the same structure as in train_kmean_torch
  list_mse = []
  for i in range(m.shape[0]):
    SqDist = np.sum((test_data - m[i])**2, axis = 1)
    list_mse.append(SqDist)
  
  list_mse_np = np.array(list_mse)
  list_mse_np_min = np.min(list_mse_np, 0)
  L_test = np.mean(list_mse_np_min)
