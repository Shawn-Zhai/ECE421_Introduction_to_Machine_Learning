# -*- coding: utf-8 -*-
"""NN

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1-1ybgpW2N3QLFS9FHdWIo3VCR7VE0lLW
"""

import numpy as np
import seaborn as sns
import matplotlib.pyplot as plt

from sklearn.datasets import load_iris
from sklearn.model_selection import train_test_split
from sklearn.neural_network import MLPClassifier
from sklearn.metrics import confusion_matrix

# Implementation of ReLU funtion
def activation(s):
    # return s if s > 0 or 0 if otherwise
    if s > 0:
        return s
    else:
        return 0

# Implementation of the derivative of ReLU
def derivativeActivation(s):
    # return 1 if s > 0 or 0 if otherwise
    if s > 0:
        return 1
    else:
        return 0

# Implementation of sigmoid function
# this gives x_L
def outputf(s):
    ret = 1 / (1 + np.exp(-s))
    return ret

# Implementation of the derivative of sigmoid function
# the derivative is: e^(-s) / (1+e^(-s))^2
def derivativeOutput(s):
    es = np.exp(-s)
    ret = es / ((1 + es) ** 2)
    return ret

# Implementation of the log loss error of Logistic Regression
def errorf(x_L,y):
    if y == 1:
        return -np.log(x_L)
    else:
        return -np.log(1 - x_L)

# Implementation of the derivative of the log loss
def derivativeError(x_L,y):
    #TODO: Fill in the return values
    if y==1:
        return -1 / x_L
    else:
        return 1 / (1 - x_L)

#forward propagation, generates a prediction based on neural network
#and input sample x
def forwardPropagation(x, weights):
    l=len(weights)+1
    currX = x
    retS=[]
    retX=[]
    retX.append(currX)

    # Forward Propagate for each layer
    for i in range(l-1):
        
        #Dot product between the layer and the weight matrix
        currS= np.matmul(np.transpose(weights[i]), currX)
        retS.append(currS)
        currX=currS
        if i != len(weights)-1:
            for j in range(len(currS)):
                #Apply the activation
                currX[j] = activation(currX[j])
            currX = np.hstack((1,currX))
        else:
            #Apply the output activation
            currX = outputf(currX)
        retX.append(currX)
    return retX,retS

#back propagation computes the gradient of error function relative to all
#to all weights in the neural network
def backPropagation(X,y_n,s,weights): 
    #x:0,1,...,L
    #S:1,...,L
    #weights: 1,...,L
    l=len(X)
    delL=[]

    # To be able to complete this function, you need to understand this line below
    # In this line, we are computing the derivative of the Loss function w.r.t the 
    # output layer (without activation). This is dL/dS[l-2]
    # By chain rule, dL/dS[l-2] = dL/dy * dy/dS[l-2] . Now dL/dy is the derivative Error and 
    # dy/dS[l-2]  is the derivative output.
    delL.insert(0,derivativeError(X[l-1],y_n)*derivativeOutput(s[l-2]))
    curr=0
    
    # Now, let's calculate dL/dS[l-2], dL/dS[l-3],...
    for i in range(len(X)-2, 0, -1): #L-1,...,0
        delNextLayer=delL[curr]
        WeightsNextLayer=weights[i]
        sCurrLayer=s[i-1]
        
        #Init this to 0s vector
        delN=np.zeros((len(s[i-1]),1))

        #Now we calculate the gradient backward
        #Remember: dL/dS[i] = dL/dS[i+1] * W(which W???) * activation
        for j in range(len(s[i-1])): #number of nodes in layer i - 1
            for k in range(len(s[i])): #number of nodes in layer i 
                #calculate delta at node j
                '''IMPORTANT: based on lecture note, the weight matrix defined
                for back propagation does not include the bias node weights,
                so must exclude the first row of weights. 

                This is why iteration starts at j + 1
                '''
                delN[j] = delN[j] + delNextLayer[k] * WeightsNextLayer[j + 1][k]
            #derivativeActivation
            delN[j] = delN[j] * derivativeActivation(sCurrLayer[j])
        delL.insert(0,delN)
    
    # We have all the deltas we need. Now, we need to find dL/dW.
    # It's very simple now, dL/dW = dL/dS * dS/dW = dL/dS * X
    g=[]
    for i in range(len(delL)):
        rows,cols=weights[i].shape
        gL=np.zeros((rows,cols))
        currX=X[i]
        currdelL=delL[i]
        for j in range(rows):
            for k in range(cols):
                #TODO: Calculate the gradient using currX and currdelL
                gL[j,k]= currdelL[k] * currX[j]
        g.append(gL)
    return g

#updates all weights in neural network based on graident descent
def updateWeights(weights,g,alpha):
    nW=[]
    for i in range(len(weights)):
        rows, cols = weights[i].shape
        currWeight=weights[i]
        currG=g[i]
        for j in range(rows):
            for k in range(cols):
                #Gradient Descent Update
                currWeight[j,k] = currWeight[j,k] - alpha * currG[j,k]
        nW.append(currWeight)
    return nW

#calculates the error for a sample based on x_L and y
def errorPerSample(X,y_n):
    return errorf(X[len(X) - 1], y_n)

#create a neural network that uses SGD to train
def fit_NeuralNetwork(X_train,y_train,alpha,hidden_layer_sizes,epochs):
    # Initialize the epoch errors
    err=np.zeros((epochs,1))
    
    # Initialize the architecture
    N, d = X_train.shape
    X0 = np.ones((N,1))
    X_train = np.hstack((X0,X_train))
    d=d+1
    L = len(hidden_layer_sizes)
    L=L+2
    
    #Initializing the weights for input layer
    weight_layer = np.random.normal(0, 0.1, (d,hidden_layer_sizes[0])) #np.ones((d,hidden_layer_sizes[0]))
    weights = []
    weights.append(weight_layer) #append(0.1*weight_layer)
    
    #Initializing the weights for hidden layers
    for l in range(L-3):
        weight_layer = np.random.normal(0, 0.1, (hidden_layer_sizes[l]+1,hidden_layer_sizes[l+1])) 
        weights.append(weight_layer) 

    #Initializing the weights for output layers
    weight_layer= np.random.normal(0, 0.1, (hidden_layer_sizes[l+1]+1,1)) 
    weights.append(weight_layer) 
    
    for e in range(epochs):
        choiceArray=np.arange(0, N)
        np.random.shuffle(choiceArray)
        errN=0
        for n in range(N):
            index=choiceArray[n]
            x=np.transpose(X_train[index])
            #Model Update: Forward Propagation, Backpropagation
            X, S = forwardPropagation(x, weights)
            g = backPropagation(X,y_train[index],S,weights)
            #update the weight and calculate the error
            weights = updateWeights(weights,g,alpha)
            errN += errorPerSample(X,y_train[index])
        err[e]=errN/N 
    return err, weights

#make a prediction based the neural network
def pred(x_n,weights):
    # prediction using the forwardPropagation function
    retX,retS= forwardPropagation(x_n,weights)
    l=len(retX)

    # Return -1 if probability lesser than 0.5
    # Else return 1
    if retX[l-1]<0.5:
        return -1
    else:
        return 1

#confusion matrix
def confMatrix(X_train,y_train,w):
    #This is a copy from PA1
    eCount=np.zeros((2,2), dtype=np.int8) # make the elements integers
    j=0
    row, col = X_train.shape
    X0 = np.ones((row,1))
    X_train = np.hstack((X0,X_train))
    for j in range(row):
        if (pred(X_train[j],w)==-1 and y_train[j]==-1):
            eCount[0,0]=eCount[0,0]+1
        elif (pred(X_train[j],w)==1 and y_train[j]==-1): 
            eCount[0,1]=eCount[0,1]+1
        elif (pred(X_train[j],w)==1 and y_train[j]==1):
            eCount[1,1]=eCount[1,1]+1
        else:
            eCount[1,0]=eCount[1,0]+1
    return eCount

#plot the neural network: error vs. epoch
def plotErr(e,epochs):
    #Plot the function using plt.plot(...,...,linewidth=2.0)
    x = np.arange(1, epochs + 1)
    y = e
    plt.title("Error vs. Epoch") 
    plt.xlabel("Epochs")
    plt.ylabel("Error")
    plt.plot(x,y,linewidth=2.0)
    plt.show()

#SciKit implementation of a MLP classifer
def test_SciKit(X_train, X_test, Y_train, Y_test):
    #Follow the tutorial in class
    NN = MLPClassifier(solver='adam', alpha = 0.00001, 
                       hidden_layer_sizes=(30, 10), random_state = 1)
    
    NN.fit(X_train, Y_train)

    y_pred = NN.predict(X_test)

    #print("Accuracy score:", NN.score(X_test, Y_test))
    cM = confusion_matrix(Y_test, y_pred)
    return cM

def test_Part1():
    from sklearn.datasets import load_iris
    X_train, y_train = load_iris(return_X_y=True)
    X_train, X_test, y_train, y_test = train_test_split(X_train[50:],y_train[50:],test_size=0.2, random_state=1)
    
    for i in range(80):
        if y_train[i]==1:
            y_train[i]=-1
        else:
            y_train[i]=1
    for j in range(20):
        if y_test[j]==1:
            y_test[j]=-1
        else:
            y_test[j]=1
        
    err,w=fit_NeuralNetwork(X_train,y_train,1e-2,[30, 10],100)
    
    plotErr(err,100)
    
    cM=confMatrix(X_test,y_test,w)
    
    sciKit=test_SciKit(X_train, X_test, y_train, y_test)
    
    print("Confusion Matrix is from Part 1a is: ",cM)
    print("Confusion Matrix from Part 1b is:",sciKit)

test_Part1()